...
- **The bed profile is per-bed data the admin keeps and the public "About this bed" page states** (`/t/<tag>/about`, reached by a small text link under BOTH doors' buttons — never a third big button).
  ...
  `plantsNote` and `recommendedPlantsNote` render only while their switch (`plantsPresent` / `plantingRecommended`) is on, and the save never blanks them, so flipping a switch back restores the words.
  `treePresent`/`plantsPresent`/`plantingRecommended` are the panel's switches; `plantsNote`/`recommendedPlantsNote`/`careNote` are admin-typed free text, bounded by `MAX_BED_NOTE_CHARS`, rendered AS TYPED in both languages (own leaf, no `data-en`/`data-es`) like `bedName`.
