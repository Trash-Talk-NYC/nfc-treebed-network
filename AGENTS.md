...
- **A counted applause can mail the bed's stewards — at most ONE notification per bed per NY day, however many people applaud** (captain, 2026-09-12: "i realize when applause is sent i dont get emailed").
  The rule is `sendApplause`'s (`Bed.applauseNoticeAt`, a claim written BEFORE any send — the digest's claim-then-send shape, so a failed send forfeits a day rather than doubling one); the words are `applause-mail.ts`; the sends happen in the route, outside the transaction, awaited in parallel because on the netlify target work after the response may never run.
  Who is mailable is the digest's own bar — an email on record and no `digestOptedOut`, ONE flag for every courtesy mail, same signed unsubscribe link — and a pen-and-paper steward with no email is never a recipient. A counted applause with nobody mailable, or with mail unconfigured, leaves the day UNCLAIMED on purpose.
