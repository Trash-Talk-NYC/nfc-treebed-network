...
- `npm run dev` / `npm run build` / `npm run preview` / `npm test` (vitest) / `npm run test:e2e` / `npm run check` (astro check).
  `npm test` is the fast suite — rules and transport handling, no build — and `npm run test:e2e` builds the app, serves `dist/server/entry.mjs`, and posts real bodies at it (`vitest.e2e.config.ts`); CI runs `npm run check` and then both (`.github/workflows/tests.yml`).
...
Four passes ride it today: `rewrite-species-casing.mjs`, `carry-steward.mjs`, `seed-captain-facts.mjs` and `retire-orphan-run-beds.mjs`.
The operator passes are no longer owed: all three ran against the live pilot store on 2026-09-12, dry run first and then `--commit`, landing as `rev/224` (seed-captain-facts — 7 fields across the four plates, `2SHFW171`'s planting recommendation left alone), `rev/225` (retire-orphan-run-beds — the two blank PR #29 rows, tap events kept on the tombstones, no adoption found) and `rev/226` (rewrite-species-casing — 12 fields across the six W 171st beds, the demo bed left as typed).
Each stays idempotent — fill-only-where-null, already-conforming rows kept, a no-op change skipping its commit — so re-running one is safe; see the named runs above, and the README for the order.
